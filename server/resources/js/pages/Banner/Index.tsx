...
export default function BannersIndex({ banners, flash }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (editingBanner) {
            put(route('banner.update', editingBanner.id), {
                ...data,
                onSuccess: () => {
                    setIsOpen(false);
                    reset();
            ...
            post(route('banner.store'), {
                ...
                onError: () => {
                    setToastMessage('Failed to create banner');
                    setToastType('error');
                    setShowToast(true);
                },
            });
    ...
    const handleDelete = (bannerId: number) => {
        destroy(route('banner.destroy', bannerId), {
            onError: () => {
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                ...
                {banners.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {banners.map((banner) => (
                            <Card key={banner.id} className="hover:bg-accent/50 transition-colors">
                                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                    <CardTitle className="text-lg font-medium">{banner.title}</CardTitle>
                                    ...
                                    {banner.images && (
                                        <div className="mt-2">
                                            <img
                                                src={banner.images}
                                                alt={banner.title}
                                                className="mt-2 w-full h-auto rounded-md max-h-48 object-cover"
